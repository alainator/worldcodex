...
COMMANDS (orchestrated multi-step workflows — user types /command):
  ...
  /civilization-build — full derivation-chain civilization pipeline with enforced checkpoints. Includes Step 4.5: Cultural Layer derivation (aesthetic tradition, performance, meaning-making, memory/dead, pre-sapient childhood, novel cultural forms)
...
SKILLS (progressive disclosure — loaded on demand):
  ...
  DESIGN: /designing-worlds (biomes, planets, climate), /designing-lore (myths, legends, eras, relics), /deriving-social-systems (derive coordination from biology/environment — includes communication medium taxonomy, terrain-to-settlement derivation, bond-allocation and congenital variation, sister-population delta-derivation, post-catastrophe resilience spectrum, cultural layer integration with Step 4.5)
